...
const emit = defineEmits(['update:modelValue', 'resetColumn', 'calStickyLeft', 'export', 'import', 'close']);

// refs
const panelSetting = ref<HTMLDivElement | null>(null);
...
const columnLabelClick = (e: MouseEvent, item: any) => {
  item.invisible = !item.invisible;
  emit('calStickyLeft');
};

const freezePanelSizeAfterShown = () => {
  const target = panelList.value;
  if (target) {
...
const showPanel = () => {
  setTimeout(() => buttonRef.value?.focus());
  setTimeout(() => freezePanelSizeAfterShown());
};
