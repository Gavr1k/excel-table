  ...
  >
    <div ref="panelFilter" class="panel-body">
      ...
      <div class="panel-content">
        ...
        <div class="panel-action">
          <div>
            <button class="panel-input-button" @click="toggleDropdown">
              {{ symbol }}
            </button>
...
const props = withDefaults(
  defineProps<Props>(),
  {
    ...
    localizedLabel: {
      ...
      listFirstNValuesOnly: (n: number) => `List first ${n} values only`,
      apply: 'Apply',
    },
    show: false,
    refId: null,
...
const symbol = computed(() => {
  switch (inputFilterCondition.value) {
    case '':
      return '≒';
    case '<=':
      return '≤';
    ...
    case '<>':
      return '≠';
    default:
      return inputFilterCondition.value;
  }
});
...
const setFilterCondition = (choice: string) => {
  showDropdown.value = false;
  inputFilterCondition.value = choice;
  nextTick(() => {
    inputFilterRef.value?.focus();
  });
};
...
const freezePanelSizeAfterShown = () => {
  const target = panelListRef.value;
  if (target) {
...
const removePanelSizeAfterHide = () => {
  const target = panelListRef.value;
  if (target) {
...
const doInputFilter = () => {
  ...
  debounceTimeout = window.setTimeout(() => {
    inputFilter.value = inputFilterRef.value?.value.trim() || '';
  }, 200);
};

const doFilter = () => {
  const filterValue =
    inputFilterCondition.value +
    (inputFilterRef.value?.value.trim() || '');
  if (columnFilterRef.value) {
    columnFilterRef.value.$el.textContent = filterValue;
    columnFilterRef.value.$emit('update:modelValue', filterValue);
  }
  hidePanel();
};

const filterPanelSelect = (opt: string) => {
  if (columnFilterRef.value) {
    columnFilterRef.value.$el.textContent = `=${opt}`;
    columnFilterRef.value.$emit('update:modelValue', `=${opt}`);
  }
  hidePanel();
};

const showPanel = async (refObj: any) => {
  columnFilterRef.value = refObj;
  inputFilter.value = '';
  inputFilterCondition.value = '';
  sortedUniqueValueList.value = [];
  if (inputFilterRef.value) {
    inputFilterRef.value.value = '';
  }

  if (columnFilterRef.value.$el.textContent !== '') {
    columnFilterRef.value.$el.textContent = '';
    columnFilterRef.value.$emit('update:modelValue', '');
    emit('calTable');
  }
  ...
  const fieldName = props.fields?.[refObj.colPos]?.name;
  if (fieldName) {
    props.table?.forEach((record) => {
  ...
  const keys = Object.keys(hash).sort((a, b) => a.localeCompare(b));
